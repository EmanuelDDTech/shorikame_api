...
export async function sendEmailSaleUpdate(saleData) {
  const { data, error } = await resend.emails.send({
    from: 'ShoriKameCards <pedidos@shorikamecards.com>',
    to: [saleData.user.email],
    subject: `ShoriKameCards - Pedido #${saleData.id} ${saleData.state}`,
    html: renderUpdateHTML(saleData),
  });
}
...
function renderUpdateHTML(saleData) {
  let htmlProductRows = ``;

  let updateStatusTitle = '';
  let updateStatusDescription = '';

  if (saleData.state === 'completado') {
    updateStatusTitle = `Pedido #${saleData.id} completado`;
    updateStatusDescription = `Tu pedido #${saleData.id} ha sido entregado.`;
  } else if (saleData.state === 'pendiente') {
    updateStatusTitle = `Pedido #${saleData.id} pendiente`;
    updateStatusDescription = `Tu pedido #${saleData.id} está siendo procesado.`;
  }

  saleData.sale_carts.forEach((item) => {
    htmlProductRows += `<tr>
                              <td>${item.product.name}</td>
                              <td>${item.quantity}</td>
                              <td>${item.price_unit}</td>
                              <td>${item.price_unit * item.quantity}</td>
                          </tr>`;
  });

  htmlProductRows += `<tr>
                              <td>${saleData.delivery_carrier.name}</td>
                              <td>N/A</td>
                              <td>${saleData.amount_shipping}</td>
                              <td>${saleData.amount_shipping}</td>
                          </tr>`;
  ...
  if (saleData.discount_amount > 0) {
    htmlProductRows += `<tr>
                              <td>Descuento</td>
                              <td>N/A</td>
                              <td>${-saleData.discount_amount}</td>
                              <td>${-saleData.discount_amount}</td>
                          </tr>`;
  }
  ...
  const htmlEmail = `<head>
              <meta charset="UTF-8">
              <meta name="viewport" content="width=device-width, initial-scale=1.0">
              <title>Actualización de Pedido #${saleData.id}</title>
              <style>
                  body {
                      font-family: Arial, sans-serif;
                      background-color: #f4f4f4;
          ...
          <body>
              <div class="container">
                  ...
                  <div class="content">
                      <h2>${updateStatusTitle}</h2>
                      <p>Hola ${saleData.user.name},</p>
                      <p>${updateStatusDescription}</p>
                      <h3>Detalles del Pedido:</h3>
                      <table class="order-details">
                          <tr>
                              <th>Producto</th>
                      ...
                      <p><strong>Total: ${saleData.amount_total}</strong></p>
                      <p>Si tienes dudas, contáctanos en <a href="mailto:shorikamecards@gmail.com">shorikamecards@gmail.com</a>.</p>
                      <p>Gracias por confiar en <strong>ShoriKameCards</strong> 🚀</p>
                  </div>
                  <div class="footer">
